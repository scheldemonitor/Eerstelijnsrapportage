...
if(refreshWaterstanden){
  ...
  refresh_waterstanden(base_path = savepath, datajaar = dataJaar)
} 
...
```{r hydroJaarEuro, eval=F, fig.cap="Jaargemiddelde hoog- en laagwaterstanden bij het Europlatform"}
...
#             caption = "De gemiddelde hoogwater (HW), laagwater (LW) en getijslag op de Noordzee, in de monding en in de Westerschelde. Hoog- en laagwater is gepresenteerd ten opzichte van NAP (voor Europlatform t.o.v. MSL). Gemiddeldes zijn berekend over alle beschikbare data. Trend en $R$ zijn berekend over de data sinds 2000.")

HW <- stationMedian(df_waterstanden, "Hoogwater in cm+NAP") %>% bind_rows(stationMedian(df_waterstanden, "Hoogwater in cm+MSL"))
LW <- stationMedian(df_waterstanden, "Laagwater in cm+NAP") %>% bind_rows(stationMedian(df_waterstanden, "Laagwater in cm+MSL"))
...
df2 <- df_waterstanden %>%
  ...
  mutate(
    `Getijslag in cm` = case_when(
      !is.na(`Hoogwater in cm+NAP`) ~ `Hoogwater in cm+NAP` - `Laagwater in cm+NAP`,
      is.na(`Hoogwater in cm+NAP`) ~ `Hoogwater in cm+MSL` - `Laagwater in cm+MSL`
    )
  ) %>%
  select(stationname, latitude, longitude, jaar,`Getijslag in cm`) %>% # `Hoogwater in cm+NAP`,`Laagwater in cm+NAP`,
  pivot_longer(cols = c(`Getijslag in cm`),   # `Hoogwater in cm+NAP`,`Laagwater in cm+NAP`,

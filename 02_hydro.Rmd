...
if(refresh) refresh_waterstanden()
...
plotLocations(df, nudge__y = -1000, nudge__x = 1000, angle = 20) +
...
df2 %>% #st_drop_geometry() %>%
  ...
  st_transform(28992) %>%
  ggplot(aes()) +
  geom_sf(data = backgroundmap, aes(fill = ADMIN), alpha = 0.5) +
...
dt <- data.frame("Station" = M2_amp$Station, "M2 amplitude" = M2_amp$Gemiddelde, "M4 amplitude" = M4_amp$Gemiddelde, "S2 amplitude" = S2_amp$Gemiddelde, "M2 fase" = M2_fase$Gemiddelde, "M4 fase" = M4_fase$Gemiddelde, "S2 fase" = S2_fase$Gemiddelde) %>%
  mutate(across(where(is.double), function(x) {signif(x,3)}))

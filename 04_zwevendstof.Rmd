...
```{r zwevendMetalen, fig.width=8, fig.height=12, fig.cap="Trends voor alle hier behandelde metalen in mg/kg drooggewicht in zwevend stof."}
parname <- c(
...
De concentraties arseen in zwevende stof nemen iets af in stroomafwaartse richting. Bij Schaar van Ouden Doel is de mediane waarde `r t1[t1$Parameter=="Arseen","Mediaan"]` mg/kg. Sinds 2011 nemen de waarden hier af. Over de gehele meetperiode is er een significante dalende trend (`r t1[t1$Parameter=="Arseen","Trend"]` mg/kg per jaar). Bij Vlissingen boei SSVH is de mediaan lager (`r t2[t2$Parameter=="Arseen","Mediaan"]` mg/kg), en er is een licht dalende trend (`r t2[t2$Parameter=="Arseen","Trend"]` mg/kg per jaar) over de hele periode die  statistisch significant is.

### Cadmium {#zwevend-cadmium}

De concentraties cadmium in zwevende stof nemen sterk af in stroomafwaartse richting (factor `r round(t1[t1$Parameter=="Cadmium","Mediaan"]/t2[t2$Parameter=="Cadmium","Mediaan"], 1)` tussen de twee stations). Bij Schaar van Ouden Doel is er een dalende trend van `r t1[t1$Parameter=="Cadmium","Trend"]` mg/kg per jaar aanwezig die statistisch significant is. De waardes bij Vlissingen nemen ook significant af, maar met een lagere snelheid (`r t2[t2$Parameter=="Cadmium","Trend"]` mg/kg per jaar).

### Chroom {#zwevend-chroom}

De concentraties chroom in zwevende stof nemen af in stroomafwaartse richting. Bij Schaar van Ouden Doel neemt over de hele periode de concentratie significant af met `r t1[t1$Parameter=="Chroom","Trend"]` mg/kg per jaar. De situatie bij Vlissingen boei SSVH toont voor de periode 1998-2011 een stabiele situatie, maar sinds 2012 liggen de waarden een stuk lager. Over de gehele dataset neemt het hoeveelheid chroom in zwevende stof af met `r t2[t2$Parameter=="Chroom","Trend"]` mg/kg per jaar. Een trend die statistisch significant is.

### Kobalt {#zwevend-kobalt}

De concentraties kobalt in zwevende stof nemen af in stroomafwaartse richting.  Bij Vlissingen is het gehalte `r t2[t2$Parameter=="Kobalt","Mediaan"]` mg/kg, en er is een lichte significante afname van `r t2[t2$Parameter=="Kobalt","Trend"]` mg/kg te zien. Bij Schaar van Ouden Doel is er geen significante trend waarneembaar en ligt het gehalte met `r t2[t2$Parameter=="Kobalt","Mediaan"]` mg/kg ongeveer `r round(t1[t1$Parameter=="Kobalt","Mediaan"]/t2[t2$Parameter=="Kobalt","Mediaan"], 1)` keer zo hoog.

### Koper {#zwevend-koper}

De concentraties koper in zwevende stof nemen sterk af in stroomafwaartse richting (ongeveer een factor `r round(t1[t1$Parameter=="Koper","Mediaan"]/t2[t2$Parameter=="Koper","Mediaan"], 1)`) . Er zijn sterke uitschieters in koperconcentraties op beide stations.De reden hiervoor is onbekend. In Schaar van Ouden Doel is desondanks een significante dalende trend te zien van `r t1[t1$Parameter=="Koper","Trend"]` mg/kg per jaar.     

### Kwik {#zwevend-kwik} 
 
De concentraties kwik in zwevende stof nemen af in stroomafwaartse richting met een factor `r round(t1[t1$Parameter=="Kwik","Mediaan"]/t2[t2$Parameter=="Kwik","Mediaan"], 1)`. Op beide stations is een lichte dalende trend zichtbaar. Deze trends zijn statistisch significant.

### Lood {#zwevend-lood}

De concentraties lood in zwevende stof nemen af in stroomafwaartse richting. Bij Vlissingen is `r round(t1[t1$Parameter=="Lood","Mediaan"]/t2[t2$Parameter=="Lood","Mediaan"], 1)` keer zo weingig lood in zwevende stof gemeten als bij Schaar van Ouden Doel. Bij Schaar van Ouden Doel is een dalende trend aanwezig, maar lijken de meetwaarden de laatste paar jaar stabiel. Bij Vlissingen boei SSVH is ook een significante daling zichtbaar, maar ook hier lijken de waarden de laatste jaren stabiel. De berekende trend over de gehele dataset is echter voor beide negatief (afname). De afname is sterker voor Schaar van Ouden Doel (`r t1[t1$Parameter=="Lood","Trend"]` mg/kg per jaar) dan bij Vlissingen (`r t2[t2$Parameter=="Lood","Trend"]` mg/kg per jaar).

### Vanadium {#zwevend-vanadium}

De concentraties vanadium in zwevend stof laten bijna geen duidelijke ruimtelijke gradiënt zien. Alleen de in de periode vanaf 2012 is de concentratie in Vlissingen duidelijk lager dan in Schaar van Ouden Doel. Bij Vlissingen boei SSVH is voor de hele periode een dalende trend van `r t1[t1$Parameter=="Vanadium","Trend"]` mg/kg per jaar met een statistische significantie. Bij Schaar van Ouden Doel is deze daling iets lager (`r t2[t2$Parameter=="Vanadium","Trend"]` mg/kg per jaar), maar ook significant.

### Zink {#zwevend-zink}

De concentraties zink in zwevende stof nemen sterk af in stroomafwaartse richting (factor `r round(t1[t1$Parameter=="Zink","Mediaan"]/t2[t2$Parameter=="Zink","Mediaan"], 1)`). Bij beide stations is er een dalende trend die significant is. Bij Schaar van Ouden Doel is deze trend sterker (`r t1[t1$Parameter=="Zink","Trend"]` mg/kg per jaar) dan bij station Vlissingen (`r t2[t2$Parameter=="Zink","Trend"]` mg/kg).
...
```{r zwevendPAKs, fig.width=8, fig.height=12, fig.width=7, fig.cap="Trends voor alle hier behandelde PAK's in mg/kg drooggewicht in zwevend stof."}
parname <- c(
...
Beide stations hebben een afnemende trend, hoewel de trend en de correlatie verschilt per station.
...
```{r zwevendPCBs, fig.height=10, fig.width=8, fig.cap="Trends voor alle hier behandelde PCB's in mg/kg drooggewicht in zwevend stof."}
parname <- c(
...
In 2012 worden de waarden van tribultyltin (TBT) bij Schaar van Ouden Doel niet meer uitgedrukt in tin, de oude metingen (wel uitgedrukt in tin) zijn door te vermenigvuldigen met factor 2,44 omgerekend naar tribultyltin. Bij Schaar van Ouden Doel dalen de gehaltes tribultyltin sterk. Bij Vlissingen boei SSVH is een dalende trend aanwezig. De waarden bij Vlissingen boei SSVH zijn lager dan bij Schaar van Ouden Doel. Bij Vlissingen boei SSVH wordt de laatste jaren zelfs de detectiegrens bereikt (3 $\mu$g/kg). Over de gehele dataset is er een duidelijke dalende trend van -1,8 tot -4,3 $\mu$g/kg te herkennen die voor beide stations een hoge correlatiecoëfficiënt heeft. Deze daling lijkt wel af te nemen met stabiliserende waardes in de laatste jaren. Voor Vlissingen was in 2017 geen data beschikbaar.
...
rounding = 3
dt <- statTable(df_tbt, parname, rounding)
knitr::kable(dt, 
             align = 'c',
...
plotTrendsLimits(df_tbt, parname)
```

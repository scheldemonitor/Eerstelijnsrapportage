...
# frozendatapath <- "Data_FysChem_bodem.csv"
# 
refreshData = F
...
df <- read_delim(fysChemBodemDataPath, delim = ",")
...
De hoeveelheid PCB's neemt af in de richting van de Noordzee. Met name bij de oostelijke stations lijken de gehaltes af te nemen in de tijd. <!-- %CHECK THIS STATEMENT! -->
...
Tributyltin is de enige organotinverbinding die wordt gemeten in het bodemsediment. In Figuur \@ref(fig:tributyltinBodem) is de hoeveelheid tributyltin over de jaren weergegeven per station. De gehaltes in 2010, 2013 en 2016 zijn sterk lager dan die van 2001 en 2004. Ruimtelijk gezien zijn er grote variaties. <!-- %CHECK THIS STATEMENT! also, data seems to be missing -->

```{r tributyltinBodem, fig.height=7, fig.cap="Tributyltin in het bodemsediment in de Westerschelde"}
...
parname = c("Tributyltin in ug/kg Standaard sediment in Bodem/Sediment", "Tributyltin in ug/kg in de fractie < 63 um drooggewicht in bodem/sediment")

plotTrendsWaterstand(df_clean, parname, locname = unique(df_clean$stationname)) + facet_wrap(~Station, ncol = 4) + theme(legend.position = "bottom", legend.direction = 'vertical')
```
...
```{r dataParameterBodem, fig.height=14, fig.width=8, fig.cap="Overzicht van gemeten parameters door de jaren heen."}
df_clean %>%
...
```{r dataStationBodem, fig.height=4, fig.cap="Overzicht van gemeten stations door de jaren heen."}
df_clean %>%

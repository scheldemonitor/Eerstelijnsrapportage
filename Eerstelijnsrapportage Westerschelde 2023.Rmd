...
## following code can be used if all plots should have the same scale. However, this is not recommenede, as all details will disappear in that case. 
# dummy <- df_waterstanden %>%
#   group_by(Jaar = year(datetime), stationname, parametername) %>%
#   mutate(Gemiddelde = case_when(
#     grepl("Hoogwater", parametername) ~ max(value),
#     grepl("Laagwater", parametername) ~ min(value)
#   ))
...
```{r hydroGemiddeldLW, fig.width = 7, fig.height = 4, fig.cap="Mediaan van laagwater per station berekend op alle beschikbare data vanaf 1998."}
...
```{r hydroGemiddeldHW, fig.width = 7, fig.height = 4, fig.cap="Mediaan van hoogwater per station berekend op alle beschikbare data vanaf 1998"}
...
knitr::kable(dt, 
             align = 'lccc',
             caption = "Jaarlijks mediaan van hoogwater, laagwater en getijslag (cm) van de Westerschelde en de monding berekend op alle beschikbare data vanaf 1998. Hoog- en Laagwater is t.o.v. NAP, behalve voor Europlatform (t.o.v. MSL).")
...
dt <- data.frame(
  "Station" = M2_amp$Station, 
  "M2 ampl" = M2_amp$Gemiddelde, 
  "M4 ampl" = M4_amp$Gemiddelde, 
  "S2 ampl" = S2_amp$Gemiddelde, 
  "M2 fase" = M2_fase$Gemiddelde, 
  "M4 fase" = M4_fase$Gemiddelde, 
  "S2 fase" = S2_fase$Gemiddelde) %>%
...
De fase van de M2-component hangt af van de positie in het estuarium, door de looptijd van het getij daar doorheen. De M2 fase is in de loop van de jaren 1998-2022 niet veel veranderd.
...
Over de getoonde periode (1998-2022) neemt de fase van de M4-component bij Hansweert en bij Bath iets af. Voor de overige stations is de fase gelijk over dze periode.
...
plotLocations(df_golven, nudge__y = -1000, nudge__x = 1000, angle = 20) +
  coord_sf(xlim = c(0, 85000), ylim = c(370000, 410000), datum=28992) +
  theme_void()
...
knitr::kable(
  ...
  caption = "Golfspectrum per meetperiode en locatie en meetinstrument in 2018."
  )
```
...
```{r gemiddeldeHs, fig.width = 7, fig.height = 4, fig.cap="Gewogen gemiddelde significante golfhoogte per station berekend op basis van het spectrum voor alle beschikbare data vanaf 1998."}
...
```{r gemiddeldezinkfil, fig.width = 7, fig.height = 4, fig.cap="Mediaan van het gefilterde zinkgehalte station berekend op alle beschikbare gegevens vanaf 1998."}
...
```{r gemiddeldezinktotaal, fig.width = 7, fig.height = 4, fig.cap="Mediaan van het totale zinkgehalte station berekend op alle beschikbare gegevens vanaf 1998."}
...
De organotinverbindingen (dibutyltin, difenyltin, monobutyltin, monofenyltin, trifenyltin, tributyltin) in mossel worden gesommeerd weergegeven. Bij de locaties Hansweert boei OHMG en Vlissingen boei SSVH (de actieve monitoring) wordt bij de metingen van difenyltin en monofenyltin regelmatig de detectiegrens bereikt. De sommatie van de waarden geeft hierdoor een hogere waarde dan in werkelijkheid (zie Tabel \@ref(tab:biotaMosselOrgano)). Er is bij deze stations wel een duidelijke afnemende trend te zien, zie Figuur \@ref(fig:mosselOrgano). Bij de locatie Knuitershoek (passieve monitoring; omgerekend van nat naar drooggewicht) is de waarde na 2015, na een uitschieter in 2014, weer gestabiliseerd rond 180 tot 200 $\mu$g/kg. Over de hele tijdserie van 2009 - nu is de afname significant. 

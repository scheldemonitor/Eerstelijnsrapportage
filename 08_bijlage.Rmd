...
df_waterstanden %>%
  ...
  geom_vline(xintercept = ymd(paste(dataJaar, "12", "31")), color = "red") +
  geom_text(x = ymd(paste(dataJaar, "12", "31"), y = NULL), label = "nu", color = "red")
...
df2 %>%
  ...
  summarize(n = mean(n), .groups = "drop") %>% filter(!grepl("nutr", category)) %>%
  # filter(grepl("PCB", parametername)) %>%
  ggplot(aes(year, as.factor(category))) +

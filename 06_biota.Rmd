...
## Stoffen in biota - overzicht van de metingen

```{r diagnostiek, fig.height=8, fig.width=10}
df2 %>%
  ...
  group_by(scientificname, category, tissue, meting) %>%
  summarize(n = mean(n)) %>%
  # filter(grepl("PCB", parametername)) %>%
  ggplot(aes(category, scientificname)) +
  geom_point(aes(size = n)) +
  coord_flip() +
  theme(
    legend.position = "top"
    ) +
  facet_grid(meting ~ tissue) +
...
# 
# metalen in vlees (o.a.) nog niet omgerekend naar droog- of natgewicht. Beide komen voor. 


df2.lever <- df2 %>% filter(grepl("lever", parametername, ignore.case = T))
df2.spier <- df2 %>% filter(grepl("spier", parametername, ignore.case = T))
df2.vlees <- df2 %>% filter(!grepl("lever", parametername, ignore.case = T) & !grepl("spier", parametername, ignore.case = T) & !grepl("gal", parametername, ignore.case = T))
...
knitr::kable(
  statTable(df3, parname = C("Arseen in mg/kg drooggewicht (vlees) in organisme (biota)"), rounding = 3),
...
plotTrends(df3, "Arseen in mg/kg natgewicht (vlees) in organisme (biota)", trend = F)
```

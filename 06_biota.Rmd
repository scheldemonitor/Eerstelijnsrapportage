...
df %>% 
  ...
  ungroup() %>%
  select(datetime, latitude, longitude, stationname, parametername, parameterunit, scientificname, class, category) %>%
  duplicated() %>%
  which() %>% 
  length()
...
df2 %>% 
  filter(scientificname %in% c("Mytilus edulis", "Platichthys flesus")) %>%
    ...
    geom_sf(aes(color = scientificname), size = 4, alpha = 0.8) + 
...
De metalen in bot worden sinds 1996 ingemeten en zijn weergegeven in Figuur \@ref(fig:botMetaal) als natgewicht. Lood neemt significant af over de jaren. Koper, kwik en zink in bot nemen significant toe in de tijd. Cadmium en kwik worden langer gemeten dan de andere metalen. Cadmium neemt iets toe, maar deze verandering is niet significant met de gebruikte methode. 
...
sumHCBdfBot <- df2_bot %>%
  ...
  left_join(vetgehalte_lever, by = c(year = "year")) %>%
  mutate(
    parametername = "sumHCH in ug/g vet",
    value2 = value/vetgehalte
  )

knitr::kable(
  statTableParams(sumHCBdfBot, parnames = "sumHCH in ug/g vet", statname = "Middelgat, Brouwersplaat, Molenplaat (deelgebied)",rounding = 3
  ), 
  align = 'c',
...
plotTrends(sumHCBdfBot, parname = "sumHCH in ug/g vet")
...
plotTrends(df2_mossel2, "Arseen in mg/kg drooggewicht (vlees) in organisme (biota)", trend = T, facet_scales = "fixed")
```
...
plotTrends(df2_mossel2, "Cadmium in mg/kg drooggewicht (vlees) in organisme (biota)", trend = F, facet_scales = "fixed")
```
...
plotTrends(df2_mossel2, "Chroom in mg/kg drooggewicht (vlees) in organisme (biota)", trend = F, facet_scales = "fixed")
```
...
plotTrends(df2_mossel2, "Koper in mg/kg drooggewicht (vlees) in organisme (biota)", trend = F, facet_scales = "fixed") +
...
plotTrends(df2_mossel2, "Kwik in mg/kg drooggewicht (vlees) in organisme (biota)", trend = F, facet_scales = "fixed")
```
...
plotTrends(df2_mossel2, "Lood in mg/kg drooggewicht (vlees) in organisme (biota)", trend = T, facet_scales = "fixed") +
...
plotTrends(df2_mossel2, "Nikkel in mg/kg drooggewicht (vlees) in organisme (biota)", facet_scales = "fixed")
```
...
plotTrends(df2_mossel2, "Zink in mg/kg drooggewicht (vlees) in organisme (biota)", facet_scales = "fixed")
```
...
plotTrends(sumPAKdfMossel, "somPAK", facet_scales = "fixed")
...
plotTrends(sumPCBdroogdfMossel, "somPCBdroog", facet_scales = "fixed")
```
...
plotTrends(sumPCBnatdfMossel, "somPCBnat", facet_scales = "fixed")
```
...
plotTrends(somPBDEMossel, "somPBDE", facet_scales = "fixed")
```
...
plotTrends(somOrganotin, parname = "somOrganotin", facet_scales = "fixed")
```
...
plotTrends(df2_mossel2, parname = "Hexachloorbenzeen in ug/kg drooggewicht (vlees) in organisme (biota)", facet_scales = "fixed") +
...
plotTrends(df2_mossel2, parname = "Hexachloorbutadieen in ug/kg drooggewicht (vlees) in organisme (biota)", facet_scales = "fixed") +
  coord_cartesian(ylim = c(0,NA))

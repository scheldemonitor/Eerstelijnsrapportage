...
# nrow(df2.lever) + nrow(df2.spier) + nrow(df2.vlees)
...
Vanaf 2017 meet RWS de prioritaire stoffen waarvoor een biotanorm is vastgesteld ook in biota. Het gaat om PBDE's. HCB, PFOS, heptachloor + -epoxide, dioxines, fluorantheen, BaP, hexachloorbutadieen, Hg, dicofol en HBCDD. Er wordt gemonitord op 18 RWS-locaties in gehele vis (blankvoorn of bot) en voor PAK's in mosselvlees van 6 weken uitgehangen mosselen. Meer informatie is te vinden in @Postma2015 en @Hoijtink2020.
...
df2 %>% group_by(stationname, scientificname) %>%
  ...
  leaflet::addLegend(pal = pal, values = c("Mytilus edulis", "Platichthys flesus"))} else{
  nudge__x = 1000
  nudge__y = -500
  angle__ = -30
df2 %>% 
  filter(scientificname %in% c("Mytilus edulis", "Platichthys flesus")) %>%
    group_by(stationname, scientificname) %>%
    summarize(latitude = mean(latitude, na.rm = T), longitude = mean(longitude, na.rm = T)) %>%
    ...
    geom_sf(aes(color = scientificname), size = 4, alpha = 0.5) + 
...
statTableParams(
  df2_bot, 
  parnames = unique(tempDF$parametername), 
  statname = unique(tempDF$stationname), 
  rounding = 2
)
...
```{r botMetaal, fig.height=8, fig.width=6, fig.show='hold', fig.cap="Jaarlijks gemiddelde (groen), minimum (blauw) en maximum (rood) van diverse metalen in bot bij Middelgat, Brouwersplaat, Molenplaat in de Westerschelde. Metaal in bot wordt sinds 1996 ingemeten (start van de grafiek)."}

# knitr::include_graphics("Figuren/Biota/1_Bot_metaal.png")

# select for classes (correct?)
# take average of classes per date (correct?)
...
vocht_mossel <- df2_mossel %>% filter(grepl("vocht", parametername, ignore.case = T)) %>% group_by(stationname) %>% summarize(mean_vocht = 0.1*round(mean(value), 0), min_vocht = 0.1*min(value), max_vocht = 0.1*max(value), n = n())

df2_mossel2 <- df2_mossel %>%
  group_by(stationname, latitude, longitude, datetime, parametername, category, class) %>% 
...
plotTrends(df2_mossel2, "Arseen in mg/kg drooggewicht (vlees) in organisme (biota)", trend = F)
```
...
plotTrends(df2_mossel2, "Cadmium in mg/kg drooggewicht (vlees) in organisme (biota)", trend = F)
```
...
plotTrends(df2_mossel2, "Koper in mg/kg drooggewicht (vlees) in organisme (biota)", trend = F) +
...
plotTrends(df2_mossel2, "Lood in mg/kg drooggewicht (vlees) in organisme (biota)", trend = F) +
  coord_cartesian(ylim = c(0,NA), xlim = c(1998, dataJaar))
```
...
PAKparams <- c(
  "Benzo(e)pyreen in ug/kg drooggewicht (vlees) in organisme (biota)",         
  "Benzo(e)pyreen in ug/kg natgewicht (vlees) in organisme (biota)",   
  "Benzo(b)fluorantheen in ug/kg drooggewicht (vlees) in organisme (biota)",   
  "Benzo(b)fluorantheen in ug/kg natgewicht (vlees) in organisme (biota)", 
  "Benzo(g,h,i)peryleen in ug/kg drooggewicht (vlees) in organisme (biota)",   
  "Benzo(g,h,i)peryleen in ug/kg natgewicht (vlees) in organisme (biota)",     
  "Benzo(k)fluorantheen in ug/kg drooggewicht (vlees) in organisme (biota)",   
  "Benzo(k)fluorantheen in ug/kg natgewicht (vlees) in organisme (biota)",  
  "Fluorantheen in ug/kg drooggewicht (vlees) in organisme (biota)",           
  "Fluorantheen in ug/kg natgewicht (vlees) in organisme (biota)",    
  "Indeno(1,2,3-c,d)pyreen in ug/kg drooggewicht (vlees) in organisme (biota)",
  "Indeno(1,2,3-c,d)pyreen in ug/kg natgewicht (vlees) in organisme (biota)"
)

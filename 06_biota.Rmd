...
```{r setupBiota, include=FALSE, message=F}
...
refreshData = F
if(refreshData) refresh_fysischchemischbiota(endyear = dataJaar) 
FyschemBiotaDataPath <- "Data_FysChem_biota.csv"

```

```{r}

df <- read_delim(file = file.path(datapath, FyschemBiotaDataPath), delim = ",")
...
df2 <- df %>% 
  ...
  filter(class %in% c('Lengte > 249 mm en Lengte < 300 mm', 'Lengte > 250 mm en Lengte < 281 mm', '"Lengte > 250 mm en Lengte < 300 mm')) %>% 
  select(FID, id,
...
# "Percentage drooggewicht in % Natgewicht (Lever) in Organisme (biota)"
...
dwnw.lever <- df2.lever %>%
              ...
              mutate(nat2droogFactor = case_when(
                parametername == "Percentage drooggewicht in % Natgewicht (Lever) in Organisme (biota)" ~ value / 100,
